...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/smartscreenone-dev-apps/${MS_NAME}:smartscreenone-dev-apps --dry-run -o yaml >> Orchestration/deployment-smartscreenone-dev.yaml'
   ...
   if(env.CODE_COVERAGE == 'True')
   {
   	stage('Code Coverage')
   	{
		FAILED_STAGE=env.STAGE_NAME
		sh 'mvn package'
		jacoco(buildOverbuild:'true',deltaBranchCoverage: '100', deltaClassCoverage: "${DELTA_CLASS_COVERAGE}", deltaComplexityCoverage: "${DELTA_COMPLEXITY_COVERAGE}", deltaInstructionCoverage: "${DELTA_INSTRUCTION_COVERAGE}", deltaLineCoverage: "${DELTA_LINE_COVERAGE}", deltaMethodCoverage: "${DELTA_METHOD_COVERAGE}")
   	}
   }
   ...
   stage('Build and Tag Image for Dev')
   {
	  ...
	  node('docker')
    {
        ...
        container('jnlp')
        {
            ...
            sh 'docker tag ${MS_NAME}:latest ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:smartscreenone-dev-apps'
			sh 'docker login ${DOCKER_REGISTRY} --username $username  --password $password'
			sh 'docker push ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:smartscreenone-dev-apps'
        }
    }
	 }
	 }  
